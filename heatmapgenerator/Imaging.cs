...
namespace heatmapgenerator
{
    class Imaging
    {
        ...
        public Imaging() { }
        public Bitmap resetWhitePixel(Bitmap originalImage)
        {
            ...
            for (x = 0; x < originalImage.Width; x++)
            {
                ...
                {
                    if ((originalImage.GetPixel(x, y)).G > 253 && (originalImage.GetPixel(x, y)).B > 253 && (originalImage.GetPixel(x, y)).R > 253)
                    {

                        originalImage.SetPixel(x, y, Color.FromArgb(0, 0, 0, 0));
                    }
                }
            }
        ...
        public Bitmap CreateIntensityMask2(Bitmap bSurface, List<HeatPoint> aHeatPoints)
        {
            ...
            foreach (HeatPoint DataPoint in aHeatPoints)
            {
                ...
                DrawHeatPoint(DrawSurface, DataPoint, Convert.ToInt32(0.50 * DataPoint.strength), DataPoint.recordtime);
              //  DrawHeatPoint(DrawSurface, DataPoint, Convert.ToInt32(0.50 * DataPoint.strength));
            }
            return bSurface;
        }
        ...
        public void DrawHeatPoint(Graphics Canvas, HeatPoint HeatPoint, int Radius, String recordtime)
        {
            ...
            if (dateprintfrequency == 1)
            {
                Font myfont = new Font("Times New Roman", 12, FontStyle.Bold);

                Canvas.DrawString(recordtime, myfont, Brushes.Black, CircumferencePoint);
            }
            else if (dateprintfrequency == 20)
            {
                dateprintfrequency = 0;
            }

            dateprintfrequency += 1;
